...
/*Had to declare these as globals since they're used in both
main() and eachThread() */
float scaled_color[width][height][3];
int nthreads =0;
scene_t scene;

void* eachThread (void *arg)
{
  ...
  int* tid = (int *)arg;
    ...
    //skip nthreads number of pixels while iterating horizontally,
    //so each thread gets width/nthreads columns.
    for( int p_x= *tid; p_x<width; p_x += nthreads)
    {
        const double x = pixel_dx * ((double)( p_x-(width/2) ));
        for( int p_y=0; p_y <height; ++p_y )
        {
            const double y = pixel_dy * ((double)( p_y-(height/2) ));
            Vec3 pixel_color;
	    ...
	    /*Moved this into the 2nd nested loop, so that it sets the colors while
	    iterating down the column */
            scaled_color[p_x][p_y][0] = gamma( pixel_color[0] ) * max_color;
            scaled_color[p_x][p_y][1] = gamma( pixel_color[1] ) * max_color;
            scaled_color[p_x][p_y][2] = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            for( int i=0; i<3; i++)
                scaled_color[p_x][p_y][i] = max( min(scaled_color[p_x][p_y][i], 255), 0);
        }
    }
...
main( int argc, char **argv )
{
    ...
    //Removed if-statement that checked if nthreads was 1.
    ...
    //Array of integer pointers to store the x coordinate.
    int* x_coord= (int*)malloc(sizeof(int)*nthreads);

    for(int i=0; i<nthreads; i++)
      x_coord[i]=0;

    //Create threads based on nthreads, using eachThread().
    int t_id;
    for(int i=0; i< nthreads ; i++)
      {
	t_id = pthread_create(&threads[i], NULL, eachThread, x_coord+i );
	if(t_id)
	  {
	    fprintf(stderr,"Could not create thread.\n");
	    return -1;
	  }
	x_coord[i+1]= x_coord[i]+1; //next thread will start one x-coordinate to right. 
      }

    //Original threads must wait for spawned threads to finish.
    for(int i=0; i< nthreads ; i++) {
	t_id = pthread_join(threads[i], NULL);
	if(t_id)
	{
    ...
    //Finally print out image after processing.
    for (int p_x=0; p_x < width ; p_x++) {
      for (int p_y=0; p_y < height; p_y++){
            printf( "%.0f %.0f %.0f\n",
		    scaled_color[p_x][p_y][0], scaled_color[p_x][p_y][1], scaled_color[p_x][p_y][2]);
      }
     }
